...
description: 사진을 업로드하고 원하는 프롬프트를 복사하여 AI에게 전달하면, **사진의 특징을 담은 빈티지 러버 스탬프 포스터**를 생성해주는 스킬입니다.
origin: content-lab
grade: A
difficulty: 초급
category: 디자인
...
💡 사진을 업로드하고 원하는 프롬프트를 복사하여 AI에게 전달하면, **사진의 특징을 담은 빈티지 러버 스탬프 포스터**를 생성해주는 스킬입니다.

## 이게 뭔가요?
이 스킬은 ChatGPT나 Gemini와 같은 AI 챗봇을 활용하여, 업로드한 사진을 기반으로 독특한 러버 스탬프 스타일의 포스터를 제작하는 방법입니다. 사진의 주요 특징을 추출하여 빈티지한 러버 스탬프 디자인으로 재해석하며, 여행 기록, 디자인 프로젝트 등에 활용할 수 있습니다.

이 스킬은 주로 **ChatGPT** 또는 **Gemini**와 같은 이미지 분석 및 생성 기능이 있는 AI 도구에서 활용됩니다. 별도의 유료 툴 없이 AI 챗봇 내에서 프롬프트 엔지니어링을 통해 구현할 수 있습니다.
...
### 2. 프롬프트 선택 및 입력
다음은 두 가지 레이아웃 옵션에 대한 프롬프트입니다. 원하는 스타일을 선택하여 복사한 후, 준비한 사진과 함께 AI 챗봇에 입력합니다.

#### 옵션 1: 세로형 3:4 - 일러스트가 상단, 사진이 하단
```
...
### 3. 결과물 확인 및 수정
AI가 생성한 결과물을 확인합니다. 원하는 결과가 나오지 않았다면, 텍스트 수정 (예: 장소명 변경) 또는 프롬프트 조정을 통해 다시 시도합니다.

## 활용 예시

**시나리오 1: 여행 기록 포스터 제작**
*   **입력:** 프랑스 파리의 에펠탑 사진 + 옵션 1 프롬프트
...
## 주의사항
*   **사진 선택:** 메인이 되는 오브젝트가 명확하고, 이미지의 해상도가 높을수록 좋은 결과물을 얻을 수 있습니다.
*   **텍스트 수정:** AI가 생성하는 텍스트는 임의적이므로, 원하는 내용이 아닐 경우 구체적으로 수정 요청해야 합니다 (예: "도쿄 대신 요코하마로 바꿔줘").
*   **보정된 사진 활용:** AI에 업로드하기 전, 사진을 미리 보정하면 결과물의 퀄리티를 높일 수 있습니다.

## 출처
[Notion | Where teams and agents work together](https://app.notion.com/p/3cb0cb665b9a80a8948eedf3973f0831?source=copy_link)
