...
description: Claude 유료 플랜(Pro $20/월~)에서 사용 가능한 공식 커넥터로 Higgsfield·Notion·Slack·Canva·Gmail·Google Drive·Calendar를 바로 연결하고, Zapier MCP를 다리로 삼아 Google Sheets·YouTube·Zoom·Telegram·Discord까지 확장하는 방법을 단계별로 정리했다. Instagram·카카오톡·네이버는 제약이 있어 조건부·직접 개발이 필요하다.
origin: content-lab
grade: S
difficulty: 중급
category: 자동화
ai_tools: ["Claude", "Canva"]
sources:
...
Claude(클로드) AI는 다양한 외부 서비스와 연동할 수 있는 **공식 커넥터**와 **MCP(Model Context Protocol)** 기능을 제공한다. 이를 통해 Notion, Slack, Canva, Gmail, Google Drive 등 자주 쓰는 앱을 Claude 안에서 직접 조작·자동화할 수 있다. 특히 Zapier MCP를 다리로 쓰면 9,000개 이상의 앱과 연동해 복잡한 워크플로우까지 구축 가능하다. 강력한 기능 대부분은 Claude 유료 플랜(Pro $20/월 이상)이 필요하며, 무료 대안으로는 Gemini·ChatGPT 등 다른 AI 도구를 기능 제한적으로 활용할 수 있다.

연동은 크게 4단계 난이도로 나뉜다.
- ✅ 바로 가능: 공식 커넥터/전용 MCP (Higgsfield, Notion, Slack, Canva, Zapier)
- 🟡 공식 커넥터지만 읽기+생성까지만: 수정·정리는 Zapier로 보완 (Gmail, Google Drive, Google Calendar)
- 🟡 Zapier/봇 토큰 설정하면 가능: Google Sheets, YouTube, Zoom, Telegram, Discord
...
1. **콘텐츠 제작 자동화**: Higgsfield 커넥터로 "'AI 활용법' 주제로 30초짜리 홍보 영상 만들어줘"라고 요청하면 Claude가 영상을 생성
2. **업무 보고 자동화**: Notion + Slack 커넥터를 함께 사용해 "주간 회의록을 Notion에 저장하고 주요 결정사항을 Slack #general 채널에 공유해줘"로 업무 효율 증대
...
- **AI 영상 콘텐츠 제작 파이프라인**: Higgsfield로 영상을 생성하고, YouTube 커넥터로 직접 업로드하며, Canva로 썸네일을 제작하는 일련의 과정을 Claude에게 맡기는 워크플로우 설계

## 주의사항

- Zapier MCP 서버 URL은 비밀번호처럼 다뤄야 하며 유출에 주의
- 대부분의 강력한 기능은 Claude 유료 플랜(Pro $20/월 이상) 필요
- 10, 13, 15번 기능(Google Sheets, YouTube, Zoom)은 16번 Zapier MCP를 먼저 설정해야 작동
- 일부 서비스(예: Instagram Insights)는 외부 유료 서비스 연동이 필요하거나 특정 계정 유형(비즈니스/크리에이터)만 지원
- 카카오톡 메시지 전송은 공식 커넥터가 없으며, 일반 사용자는 개인 메모 기능 외에 타인에게 메시지를 보내는 것이 사실상 불가능
